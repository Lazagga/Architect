...
module CPU(
	...
	always @(*) begin
		if (SavePC)						wr_addr = 5'd31;
		else if (RegDst)				wr_addr = rd;
		else							wr_addr = rt;

		if (SavePC)						wr_data = PC + 4;
		else if (MemtoReg)				wr_data = mem_read_data;
		else							wr_data = alu_result;

		if (halt)						PC_next = PC;
		else if (JR)					PC_next = rd_data1;
		else if (Branch & alu_result)	PC_next = PC + 4 + {ext_imm[29:0], 2'b00};
		else if (Jump)					PC_next = {PC[31:28], immj, 2'b00};
		else							PC_next = PC + 4;
	end
